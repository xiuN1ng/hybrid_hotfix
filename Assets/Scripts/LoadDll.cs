...
public class LoadDll : MonoBehaviour
{
    ...
    IEnumerator InitYooAssets(Action onDownloadComplete)
    {
        ...
        else if (PlayMode == EPlayMode.HostPlayMode)
        {
            ...
            {
                ColorLog.YellowLog("资源包初始化成功！");
            }
            else
            {
                ColorLog.LogError($"资源包初始化失败：{initOperation.Error}");
            }
        }
    ...
    IEnumerator Download()
    {
        ...
        {
            //下载成功
            ColorLog.YellowLog("更新完成");
        }
        else
        {
            //下载失败
            ColorLog.YellowLog("更新失败");
        }
    }
    ...
    {
        ColorLog.YellowLog(string.Format("开始下载：文件名：{0}，文件大小：{1}", fileName, sizeBytes));
    }
    ...
    {
        ColorLog.YellowLog("下载" + (isSucceed ? "成功" : "失败"));
    }
    ...
    {
        ColorLog.YellowLog(string.Format("文件总数：{0}，已下载文件数：{1}，下载总大小：{2}，已下载大小{3}", totalDownloadCount, currentDownloadCount, totalDownloadBytes,
    ...
    {
        ColorLog.YellowLog(string.Format("下载出错：文件名：{0}，错误信息：{1}", fileName, error));
    }
